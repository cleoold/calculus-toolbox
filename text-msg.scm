...
(define derivative-intro-msg
  (string-append
   ...
   "  lastly, the performance second derivate depends on largely on the number of digits in your `precision` input "
   "and your function. do not use numbers that is too small (eg twenty 0s in front of 1). try multiple precisions.\n\n"))
...
(define recsequence-main-intro-msg
  (string-append
   ...
   "      a(2) = f[a(1), k=2],\n"
   "      ...,\n"
   "      a(k) = f[a(k-1), k], where f is a function that defines values recursively. "
   "\n"
   "  for example, if you want to compute the sequence {a(0) = 1, a(k) = 2 * a(k-1)}, you need to input `1` for A, and `(* 2 x)` for the "
   "function. this yields a geometric sequence of ratio 2. BE SURE to include `x` to represent your previous term, and `k` for current term index. "
   "\n"
   ...
   "function. this produces a fibonacci sequence. also be sure to include `x` for the second-previous term (k-2), and `y` for previous term (k-1),"
   " `k` for current index."
   "\n"
